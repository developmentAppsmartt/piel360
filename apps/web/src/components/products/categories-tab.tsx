...
export function CategoriesTab() {
  ...
  // ─── Columnas dinámicas (necesitan acceder a los handlers) ────────────────

  const columns = [
    col.accessor("categoryName", { header: "Nombre" }),
    col.accessor((row) => row._count?.products ?? 0, {
      id: "products",
      header: "Productos",
      cell: (info) => (
      ...
      {deleteTarget && (
        <Dialog open onOpenChange={() => setDeleteTarget(null)}>
          <DialogContent>
            ...
            <p className="text-sm text-muted-foreground">
              Se eliminarán también todos los productos de{" "}
              <strong>&ldquo;{deleteTarget.categoryName}&rdquo;</strong>. Esta acción
...
// ─── Sub-componente editar (usa su propio hook de mutación) ──────────────────
